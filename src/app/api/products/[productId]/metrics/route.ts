			...
			try {
				...
				const pageViewTrends = await prisma.analyticsEvent
					...
					.then((groups) => {
						...
						groups.forEach((g) => {
							...
							map.set(
								day,
								(map.get(day) || 0) + g._count.timestamp
							);
						});
				...
				const recentThreads = await prisma.message
					...
					.then((msgs) =>
						msgs.map((m) => {
							const threadTs =
								(m.attributes as Record<string, any>)
									?.thread_ts || m.id;
							return {
								id: threadTs,
				...
				const messageTrendData = await (async () => {
					if (range === "7d") {
						...
						const weekStarts = Array.from(
							{ length: weeks },
							(_, i) => subWeeks(new Date(), weeks - 1 - i)
						);
						const counts = await Promise.all(
