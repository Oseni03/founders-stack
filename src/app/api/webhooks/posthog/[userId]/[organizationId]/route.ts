...
// ============================================================================
// BATCH WEBHOOK HANDLER (If PostHog sends batched events)
// ============================================================================

/**
 * Handle batch of PostHog events
 * Some PostHog webhook configurations send multiple events in one request
 */
export async function handleBatchWebhook(
	...
	try {
		...
		// Body can be array of events or single event
		const events = Array.isArray(body) ? body : [body];

		console.log(`Received batch of ${events.length} PostHog events`);

		// Process all events
		await Promise.all(
			events.map((event) =>
				processPostHogWebhook(event, integration, organizationId).catch(
					(error) => {
						console.error(
							`Failed to process event ${event.uuid}:`,
							error
						);
		...
		// Update last sync time once for batch
		await prisma.integration.update({
			where: { id: integration.id },
			data: { lastSyncAt: new Date() },
		});
