...
export default function CommunicationPage() {
	...
	const fetchMetrics = async () => {
		try {
			...
			const response = await fetch(
				`/api/products/${productId}/communication?range=30d`
			);
	...
	if (loading) {
		return <CommunicationPageLoading productId={productId as string} />;
	}

	if (!data) {
		return <CommunicationNoDataState productId={productId as string} />;
	}
