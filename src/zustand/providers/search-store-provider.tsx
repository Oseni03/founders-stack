...
import { SearchState, createSearchStore } from "../stores/search-store";
...
export const useSearchStore = <T,>(selector: (store: SearchState) => T): T => {
	const searchStoreContext = useContext(SearchStoreContext);

	if (!searchStoreContext) {
		throw new Error(
			`useSearchStore must be used within SearchStoreProvider`
		);
	}
